...
import { ChevronRight, X, MapPin, Phone, Clock, CreditCard, AlertCircle, Check, Loader2, ShoppingCart, ChevronLeft } from 'lucide-react';
import {
    getAddresses,
    createAddress,
    createBooking,
...
export default function Checkout() {
    ...
    const generateAvailableDates = () => {
        ...
        for (let i = 0; i < slotConfig.daysToShow; i++) {
    ...
    const generateTimeSlotsForDate = (selectedDateObj) => {
        const now = new Date();
        const slots = [];

        slotConfig.slots.forEach((slot, index) => {
            const [startHour, startMin] = slot.start.split(':').map(Number);

            const slotStartTime = new Date(selectedDateObj.date);
            slotStartTime.setHours(startHour, startMin, 0, 0);

            const bufferTime = new Date(now.getTime() + slotConfig.bufferMinutes * 60000);
            ...
            if (selectedDateObj.isToday) {
                if (slotStartTime <= bufferTime) {
                    isAvailable = false;
                    unavailableReason = 'Time passed';
                }
            }

            slots.push({
                id: `${selectedDateObj.dateString}-${index}`,
                date: selectedDateObj.dateString,
                displayDate: `${selectedDateObj.displayLabel}, ${selectedDateObj.displayDate}`,
                time: slot.label,
    ...
    const initializeTimeSlots = () => {
        ...
        if (dates.length > 0) {
            ...
            setTimeSlots(generateTimeSlotsForDate(dates[0]));
        }
    };

    const handleDateSelect = (dateObj) => {
        setSelectedDate(dateObj);
        setTimeSlots(generateTimeSlotsForDate(dateObj));
            ...
            {showSlotModal && (
                <div className="fixed inset-0 bg-black/50 z-50 flex items-center justify-center p-4">
                    <div className="bg-white rounded-2xl max-w-md w-full max-h-[90vh] overflow-hidden">
                            ...
                            <div className="flex gap-2 overflow-x-auto pb-2 scrollbar-hide">
                                {availableDates.map((dateObj) => (
                                    <button
                                        ...
                                        className={`flex-shrink-0 px-4 py-3 rounded-lg border-2 text-center min-w-[80px] transition ${selectedDate?.dateString === dateObj.dateString
                                                ? 'border-purple-600 bg-purple-50'
                                                : 'border-gray-200 hover:border-gray-300'
                                            }`}
                                    >
                                        <p className={`text-xs font-medium ${selectedDate?.dateString === dateObj.dateString
                                                ? 'text-purple-600'
                                                : 'text-gray-500'
                                            }`}>
                                            {dateObj.displayLabel}
                                        </p>
                                        <p className={`text-sm font-semibold mt-1 ${selectedDate?.dateString === dateObj.dateString
                                                ? 'text-purple-700'
                                                : 'text-gray-900'
                                            }`}>
                                            {dateObj.displayDate}
                        ...
                        <div className="p-4 overflow-y-auto max-h-[400px] scrollbar-hide">
                            <p className="text-sm text-gray-600 mb-3">
                                Available Slots for {selectedDate?.displayLabel}, {selectedDate?.displayDate}
                            </p>
                                ...
                                {timeSlots.map((slot) => (
                                    ...
                                    >
                                        <div className="flex justify-between items-center">
                                            <div className="flex items-center gap-3">
                                                <div className={`w-10 h-10 rounded-full flex items-center justify-center ${slot.available
                                                        ? 'bg-green-100'
                                                        : 'bg-gray-200'
                                                    }`}>
                                                    <Clock className={`w-5 h-5 ${slot.available
                                                            ? 'text-green-600'
                                                            : 'text-gray-400'
                                                        }`} />
                                                </div>
                                                <div>
                                                    <p className={`font-semibold ${slot.available
                                                            ? 'text-gray-900'
                                                            : 'text-gray-500'
                                                        }`}>
                                                        {slot.time}
                        ...
                        <div className="p-4 border-t bg-gray-50">
                            <p className="text-xs text-gray-500 text-center">
                                Slots are subject to availability. Book early to secure your preferred time.
                            </p>
                        </div>
                    </div>
                </div>
